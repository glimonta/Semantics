...
(** Score: 2/10
  6.1: Def ok, wrong statements proven: 1/5
  6.2: Def off by one, no lemmas proved (but attempted): 1/5 
**)
...
(**
  Oops, that's not what you wanted to prove: You have proven:
    If all programs are while-free, then all programs terminate
  which trivially holds, as there are non-while-free programs
**)

lemma [simp]: "(\<forall>c. (whileFree c)) \<longleftrightarrow> False"
  by (auto intro: exI[where x="WHILE b DO c" for b c])
...
(** Same problem here: You have proven:
  If there is a state s, for which all programs do not terminate, then
  some program c is not while-free.
**)

lemma foo: "(\<forall>c. \<not>(\<exists> t. (c,s) \<Rightarrow> t)) \<longleftrightarrow> False"
  by auto

lemma "(\<forall>c. \<not>(\<exists> t. (c,s) \<Rightarrow> t)) \<Longrightarrow> \<not>(whileFree c)" by (subst (asm) foo) blast
(**
proof (induction c arbitrary: s)
print_cases
case SKIP
...
value "cnv_to_rel [ADD, LOADI 3, JMP 0, JMPLESS 1]"
  (** You're off by one here, so you won't be able to prove the lemmas! **)
